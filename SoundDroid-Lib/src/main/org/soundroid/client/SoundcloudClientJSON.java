...
import org.soundroid.models.Events;
...
public class SoundcloudClientJSON {
	...
	// CONSTRUCTORS
	public SoundcloudClientJSON() {
		this(new DefaultHttpClient(), null);
	}

	public SoundcloudClientJSON(ClientSettings cs) {
		this(new DefaultHttpClient(), cs);

	}

	public SoundcloudClientJSON(final HttpClient hClient) {
		this(hClient, null);
	}

	public SoundcloudClientJSON(final HttpClient hClient, ClientSettings cs) {
	...
	public Response<User> getInfoAboutMe() {
		Response<User> response = (Response<User>) sendSoundcloudRequest("GET",
				"/me.json", SoundroidConstants.ME, null, this
	...
	public Response<Tracks> getTracks() {
		Response<Tracks> response = (Response<Tracks>) sendSoundcloudRequest(
				"GET", "/me/tracks.json", SoundroidConstants.TRACKS, null, this
						.getUserSpecificAccessToken());
		return response;
	}
	...
	public Response<Comments> getComments() {
		Response<Comments> response = (Response<Comments>) sendSoundcloudRequest(
				"GET", "/me/comments.json", SoundroidConstants.COMMENTS, null,
				this.getUserSpecificAccessToken());
		return response;
	}
	...
	public Response<Users> getContacts() {
		Response<Users> response = (Response<Users>) sendSoundcloudRequest(
				"GET", "/me/contacts.json", SoundroidConstants.CONTACTS, null,
				this.getUserSpecificAccessToken());
		return response;
	}
	...
	public Response<Events> getEvents(String filterValue) {
		...
		Response<Events> response = (Response<Events>) sendSoundcloudRequest(
				"GET", "/events.json", SoundroidConstants.EVENTS, params, this
						.getUserSpecificAccessToken());
	...
	protected Response<?> sendSoundcloudRequest(String method, String service,
			int requestType, java.util.Map<String, String> params, Token token) {
		checkUserAccessToken();

		String jsonResponse = sendHttpRequest(this.getClientSettings()
				.getWebServiceUrl()
				+ service, method, params, token);
		
		Response<?> r = fromJson(jsonResponse, requestType);
	...
	protected String sendHttpRequest(String baseUrl, String method,
		...
		try {
			responseMsg = client.invoke(access, method, baseUrl, params
					.entrySet());
	...
	private User parseUser(String user) {
		...
		if(user != null){			
			userDTO = new Gson().fromJson(user, User.class);
		}		

		return userDTO;
	}
	
	private Tracks parseTracks(String tracks) {
		Tracks tracksDTO = null;
		
		if(tracks != null){
			Track[] arrayTracks = new Gson().fromJson(tracks, Track[].class);
			
			tracksDTO = new Tracks();
			ArrayList<Track> listTracks= new ArrayList<Track>();	
			
			for(int i = 0; i < arrayTracks.length; i++){
				listTracks.add(arrayTracks[i]);
			}
			
			tracksDTO.setTracks(listTracks);
		}		
